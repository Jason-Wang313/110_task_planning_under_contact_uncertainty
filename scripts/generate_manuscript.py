...
def write_gate_table(summary):
    ...
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Frozen Paper 110 terminal gates. All local empirical gates pass, while the external robot/high-fidelity scope gate deliberately fails.}",
        r"\begin{tabular}{ll}",
        r"\toprule",
        r"Gate & Outcome \\",
        r"\midrule",
    ...
    for key in keys:
        value = "pass" if summary["gates"][key] else "fail"
        lines.append(f"{latex_escape(key.replace('_', ' '))} & {latex_escape(value)} \\\\")
...
def write_row_table(summary):
    labels = {
        ...
        "main_group_rows": "task/regime/split/method aggregates",
    ...
    for key, label in labels.items():
        lines.append(f"{latex_escape(label)} & {int(summary['row_counts'][key])} \\\\")
    lines += [r"\bottomrule", r"\end{tabular}", r"\end{table}"]
...
def write_failure_table():
    ...
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Representative hard-split failure cases. The machine-readable CSV contains 24 cases used to shape the terminal decision.}",
        r"\scriptsize",
        r"\resizebox{\linewidth}{!}{%",
        r"\begin{tabular}{llllrr}",
        r"\toprule",
        r"ID & Task & Regime & Lesson & Oracle gap & Damage \\",
        r"\midrule",
    ]
    for row in rows:
        lines.append(
            f"{int(float(row['case_id']))} & {latex_escape(row['task'])} & {latex_escape(row['regime'])} & "
            f"{latex_escape(row['lesson'])} & {fnum(row['success_gap_to_oracle'], 3)} & {fnum(row['damage_rate'], 3)} \\\\"
        )
    lines += [r"\bottomrule", r"\end{tabular}", "}", r"\end{table}"]
...
def write_references():
    references = r"""@inproceedings{kaelbling2011hierarchical,
...
def manuscript_template(summary):
    ...
    replacements = {
        ...
        "@@ORACLE_SUCCESS@@": fnum(oracle["success"], 3),
        "@@ORACLE_UTILITY@@": fnum(oracle["utility"], 3),
        "@@SUCCESS_MARGIN@@": fnum(gates["success_margin_vs_strongest"], 3),
        "@@UTILITY_MARGIN@@": fnum(gates["utility_margin_vs_strongest"], 3),
        "@@PRECOND_DELTA@@": fnum(gates["precondition_violation_delta_vs_strongest"], 3),
        "@@ECE_DELTA@@": fnum(gates["belief_ece_delta_vs_strongest"], 3),
        "@@RECOVERY_DELTA@@": fnum(gates["recovery_success_delta_vs_strongest"], 3),
        "@@DAMAGE_DELTA@@": fnum(gates["damage_rate_delta_vs_strongest"], 3),
        "@@WASTED_DELTA@@": fnum(gates["wasted_action_delta_vs_strongest"], 3),
        "@@COMMIT_DELTA@@": fnum(gates["irreversible_commitment_delta_vs_strongest"], 3),
        ...
        "@@STRONGEST@@": latex_escape(summary["strongest_non_oracle_baseline"]),
    }
...
On the held-out combined contact-uncertainty split, \method{} obtains success @@SUCCESS@@ and utility @@UTILITY@@. The strongest non-oracle baseline is @@STRONGEST@@ with success @@BASE_SUCCESS@@ and utility @@BASE_UTILITY@@. The success margin is @@SUCCESS_MARGIN@@ and the utility margin is @@UTILITY_MARGIN@@. The hybrid oracle reaches success @@ORACLE_SUCCESS@@ and utility @@ORACLE_UTILITY@@, so the benchmark retains meaningful headroom.
...
The full v5 method clears the best removed-component variant by success margin @@ABLATION_SUCCESS@@ or utility margin @@ABLATION_UTILITY@@. The success margin is deliberately modest because the strongest ablations still retain many useful components. The utility margin is larger because removed components tend to increase damage, invalid preconditions, intervention cost, or branch blowup even when raw success remains close.
...
The fixed-risk result could be trivial. & The strict budget was tightened into the observed risk-score range; coverage is @@FIXED_COVERAGE@@, not 1.0. \\
The oracle gap is unresolved. & Correct. Oracle success @@ORACLE_SUCCESS@@ and utility @@ORACLE_UTILITY@@ remain higher; this motivates external sensing and validation. \\
