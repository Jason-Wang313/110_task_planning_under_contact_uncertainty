        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Task Planning Under Contact Uncertainty: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study task and motion planning. The submission-hardened claim is intentionally narrow: Plan over uncertainty in future contact modes rather than over abstract symbolic preconditions. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Task planning under contact uncertainty should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.410 $\pm$ 0.015 & 0.443 $\pm$ 0.019 & 0.328 $\pm$ 0.017 \\
Data-augmented WAM & 0.501 $\pm$ 0.022 & 0.532 $\pm$ 0.014 & 0.429 $\pm$ 0.017 \\
Scalar uncertainty & 0.571 $\pm$ 0.015 & 0.590 $\pm$ 0.010 & 0.509 $\pm$ 0.015 \\
Ensemble disagreement & 0.588 $\pm$ 0.016 & 0.612 $\pm$ 0.014 & 0.539 $\pm$ 0.022 \\
Conformal risk filter & 0.615 $\pm$ 0.025 & 0.637 $\pm$ 0.007 & 0.563 $\pm$ 0.011 \\
Proposed branch mechanism & 0.700 $\pm$ 0.013 & 0.700 $\pm$ 0.013 & 0.660 $\pm$ 0.014 \\
Oracle branch upper bound & 0.771 $\pm$ 0.014 & 0.773 $\pm$ 0.010 & 0.746 $\pm$ 0.008 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.676 $\pm$ 0.011 & all components \\
Minus branch atlas & 0.563 $\pm$ 0.015 & removes explicit unrealized futures \\
Minus tail risk & 0.580 $\pm$ 0.013 & uses mean predicted value only \\
Minus diagnostic probe & 0.609 $\pm$ 0.013 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.621 $\pm$ 0.009 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/110.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Task Planning Under Contact Uncertainty is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
